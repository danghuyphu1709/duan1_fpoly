...
class DispatcherTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testReverseRouteWithDashes()
    {
        ...
        $r->any( array('products-catalogue/store/{store:i}?', 'products'), array(__NAMESPACE__.'\\Test','route'));

        $this->assertEquals('products-catalogue/store', $r->route('products'));
        $this->assertEquals('products-catalogue/store/1', $r->route('products', array(1)));
    }
    ...
    public function testPrefixGroupsReverseRoutes()
    {
        ...
        $r->group(['prefix' => 'products-catalogue/store'], function($r) {
            $r->any(['items/{store:i}?', 'products'], array(__NAMESPACE__.'\\Test','route'));
        });

        $this->assertEquals('products-catalogue/store/items', $r->route('products'));
        $this->assertEquals('products-catalogue/store/items/1', $r->route('products', array(1)));
    }
